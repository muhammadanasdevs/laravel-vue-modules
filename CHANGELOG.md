...
All Notable changes to `laravel-vue-modules` will be documented in this file.
...
- Fixed bug: `Target class [Muhammadanasdevs\Modules\Commands\] does not exist.`
...
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Muhammadanasdevs\Modules\Module`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Muhammadanasdevs\Modules\Module`.
- **BREAKING** Renamed method `enabled` to `isEnabled` in `\Muhammadanasdevs\Modules\FileRepository`.
- **BREAKING** Renamed method `disabled` to `isDisabled` in `\Muhammadanasdevs\Modules\FileRepository`.
- **BREAKING** Removed the `__get` magic method on the `\Muhammadanasdevs\Modules\Module` class. Use `get()` or `json()->get()` instead.
...
- updated readme on how to install laravel-vue-modules in Lumen
